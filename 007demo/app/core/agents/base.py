...
class BaseSubAgent:
    ...
    """
    def __init__(self, task: Task, blackboard: SharedBlackboard, tools: List[Any], session_id: str = None, system_prompt: str = EXECUTION_SYSTEM_PROMPT):
        self.task = task
    ...
    async def run(self) -> Dict:
        ...
        print(f"  ▶️ [{self.agent_name}] Starting Task {self.task.id}: {self.task.description}")
        ...
        self.memory.chat_memory.add_user_message(context_content)
            ...
            try:
                ...
                if response.tool_calls:
                    for tool_call in response.tool_calls:
                        ...
                        if selected_tool:
                            print(f"    🛠️ [{self.agent_name}] Calling {selected_tool.name}...")
                            ...
                            if self.recorder:
                                ...
                                if len(result_str) > 500 or isinstance(tool_result, (dict, list)):
                                    output_filename = f"tool_{self.task.id}_{_ + 1}_{tool_call['name']}_{tool_call['id'][:8]}.txt"
                                    # Use executions subdirectory for tool outputs to keep root clean
                                    full_output_path = f"executions/{output_filename}" 
                                    self.recorder.log_data(full_output_path, tool_result)
                    ...
                    print(f"  ✅ [{self.agent_name}] Finished Task {self.task.id}. Content: {content[:100]}...")
